...
var unitActions = 
{
	...
	"attack-move": 
	{
		...
		"hotkeyActionCheck": function(target, selection)
		{
			...
			if (haveUnitAI && Engine.HotkeyIsPressed("session.attackmove") && getActionInfo("attack-move", target).possible)
				return {"type": "attack-move", "cursor": "action-attack-move", "context" : "attack"};
			return false;
		},
		"specificness": 30,
	},
	...
	"capture": 
	{
		...
		"getActionInfo": function(entState, targetState)
		{
			if (!entState.attack || !targetState.hitpoints)
				return false;
			return {"possible": Engine.GuiInterfaceCall("CanCapture", {"entity": entState.id, "target": targetState.id, "context" : "attack"})};
		},
		"actionCheck": function(target)
		{
			if (getActionInfo("capture", target).possible)
				return {"type": "capture", "cursor": "action-capture", "target": target, "context" : "attack"};
			return false;
		},
		"specificness": 9,
	},
	...
	"attack":
	{
		...
		"hotkeyActionCheck": function(target)
		{
			if (Engine.HotkeyIsPressed("session.attack") && getActionInfo("attack", target).possible)
				return {"type": "attack", "cursor": "action-attack", "target": target, "context" : "attack"};
			return false;
		},
		"actionCheck": function(target)
		{
	...
	"heal": 
	{
		...
		"actionCheck": function(target)
		{
			if (getActionInfo("heal", target).possible)
				return {"type": "heal", "cursor": "action-heal", "target": target, "context" : "heal"};
			return false;
		},
		"specificness": 7,
	},
	...
	"build": 
	{
		...
		"actionCheck": function(target)
		{
			if (getActionInfo("build", target).possible)
				return {"type": "build", "cursor": "action-build", "target": target, "context" : "repair"};
			return false;
		},
		"specificness": 3,
	},
	...
	"repair": 
	{
		...
		{
			if (getActionInfo("repair", target).possible)
				return {"type": "build", "cursor": "action-repair", "target": target, "context" : "repair"};
			return false;
		},
		"specificness": 11,
	},
	...
	"gather": 
	{
		...
		"getActionInfo":  function(entState, targetState)
		{
			...
			if (resource)
				return {"possible": true, "cursor": "action-gather-" + resource, "type" : targetState.resourceSupply.type };
			return false;
		},
		"actionCheck": function(target)
		{
			...
			if (!actionInfo.possible)
				return false;
			return {"type": "gather", "cursor": actionInfo.cursor, "target": target, "context" : "gather", "contextData" : { "type": actionInfo.type }};
		},
		"specificness": 1,
	},
...
var g_EntityCommands = 
{
	...
	"alert": {
		"getInfo": function(entState)
		{
			if (!entState.alertRaiser || (entState.alertRaiser.hasRaisedAlert && !entState.alertRaiser.canIncreaseLevel))
				return false;
			if (entState.alertRaiser.hasRaisedAlert)
			{
				return {
					"tooltip": translate("Increase the alert level to protect more units"),
					"icon": "bell_level2.png"
				};
			}
			else
			{
				return {
					"tooltip": translate("Raise an alert!"),

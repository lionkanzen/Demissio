...
GuiInterface.prototype.GetExtendedEntityState = function(player, ent)
{
	...
	if (cmpCost && cmpCost.GetPopBonus())
		ret.populationBonus = cmpCost.GetPopBonus();
	...
	if (cmpResourceGatherer)
	{
		...
		ret.resourceCapacities = cmpResourceGatherer.GetCapacities();
	}
	...
	var cmpBuilder = Engine.QueryInterface(ent, IID_Builder);
	if (cmpBuilder)
		ret.builderRate = cmpBuilder.GetRate();
	...
	if (cmpHeal)
	{
		let rate = cmpHeal.GetHealingPower() / (cmpHeal.GetTimers().repeat / 1000);
		ret.healer = { 
			"unhealableClasses": cmpHeal.GetUnhealableClasses(),
			"healableClasses": cmpHeal.GetHealableClasses(),
			"maxRange": cmpHeal.GetRange()["max"],
			"rate": rate
		};
	}
